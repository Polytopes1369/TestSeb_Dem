...
namespace maths {
    ...
    struct mat4 {
        ...
        // General 4x4 inverse via the classic cofactor/adjugate expansion (the standard "MESA GLU"
        // formula) -- works for ANY invertible 4x4 matrix, not just the affine (translate/rotate)
        // ones every other helper above produces, because renderer::ScreenTracePass needs to invert
        // a full view * projection matrix (a genuinely non-affine, perspective-divide-carrying
        // matrix) to reconstruct a world-space position from a screen pixel + depth-buffer value --
        // the first consumer of a general matrix inverse in this codebase. A singular (non-
        // invertible) input returns the identity matrix rather than dividing by zero -- a degenerate
        // view/projection should never occur in practice (it would mean the camera has a zero-volume
        // frustum), so this is a defensive fallback, not a real code path.
        inline mat4 Inverse() const {
            ...
            if (std::abs(det) < 1.0e-12f) {
                return mat4{}; // Singular input -- see this method's own comment on why this is a defensive fallback only.
            }
