...
namespace renderer::debug {
    ...
    void DebugBufferViewPass::Init(VkDevice device, VmaAllocator allocator, VkCommandPool commandPool, VkQueue queue, VkExtent2D outputExtent, VkBuffer exposureStateBuffer) {
        ...
        // =====================================================================================
        {
            ...
            // STORAGE_BIT: this pass' own imageStore (DebugBufferView.comp). TRANSFER_SRC_BIT: the
            // final swapchain blit sources from this image whenever a Buffer Viewer entry is
            // selected. COLOR_ATTACHMENT_BIT: pre-existing bug fix (found while adding subtask E3's
            // new entry 15 and actually exercising this path for the first time -- ClusterRenderPipeline
            // ::RecordFrame's own [13b] HUD-overlay-selection block has ALWAYS branched to draw
            // renderer::debug::DebugTextOverlay's stat overlay directly onto GetOutputView() whenever
            // ANY Buffer Viewer index is selected (see that call site's own "blitSourceImage ==
            // m_DebugBufferView.GetOutputImage()" branch), but this image was never actually created
            // with the COLOR_ATTACHMENT_BIT usage that overlay draw's own vkCmdBeginRendering
            // requires -- every prior entry (1-14) hit the exact same
            // VUID-VkRenderingInfo-colorAttachmentCount-06087 validation error this fix resolves,
            // it was simply never caught before since nothing in --test-pipeline ever selects a
            // nonzero SELECTED_BUFFER_INDEX. Matches renderer::PostProcessPass's own imageInfo.usage
            // comment/fix for the identical requirement on ITS output image.
            imageInfo.usage = VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT;
