...
namespace renderer {
    ...
    namespace {
        ...
        std::vector<char> ReadShaderFile(const std::string& filename) {
            ...
            if (!file.is_open()) {
                throw std::runtime_error("ScreenTracePass: failed to open SPIR-V file: " + filename);
            }
        ...
        // Byte-for-byte mirror of ScreenTraceViewParamsUBO in ScreenTrace.comp (std140). Both
    ...
    void ScreenTracePass::Init(VkDevice device, VmaAllocator allocator, VkCommandPool commandPool, VkQueue queue,
        ...
        // --- Output image: the noisy, combined (SSRT near-field + World Probe far-field
        // fallback) GI signal renderer::ATrousDenoisePass reads next. ---
        ...
        // one-shot pattern) -- stays GENERAL for this image's entire lifetime.
        {
        ...
        m_ViewParamsBuffer.Create(allocator, sizeof(ScreenTraceViewParams),
        ...
        VkShaderModule shaderModule = CreateShaderModule(m_Device, ReadShaderFile("shaders/ScreenTrace.comp.spv"));
