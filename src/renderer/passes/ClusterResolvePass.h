...
namespace renderer {
    ...
    public:
        ...
        // comment) -- bound read-only at binding 25 (the first free slot past this shader's full
        // 0-24 range) so ClusterResolve.comp can re-derive the same spline-bent triangle both
        // rasterizers already drew (Phase 1, Nanite advanced). Retained (like entityDataBuffer) so
        // InitBinnedResolve() below needs no duplicate parameter for it.
