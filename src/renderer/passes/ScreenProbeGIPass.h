...
//      Writes this pass' OWN output image (GetOutputView(), see F9's own note below), NOT a
//      read-modify-write of any other pass' color -- the caller's own barrier after this call
//      (before renderer::GICompositePass reads GetOutputView()) must cover this pass' own
//      COMPUTE_SHADER / SHADER_STORAGE_WRITE access.
//
...
// this copy is necessary for correctness, not just an optimization).
//
// --- F9 (UE5.8 parity roadmap): restored as the HighQuality GI mode's near-field gather ---
// This class (+ its 4 shaders + screen_probe_view_params.glsl) was deleted outright by an earlier
// perf pass (see renderer::ClusterRenderPipeline.h's own tombstone-turned-restoration-note) after
// being replaced by renderer::ScreenTracePass/GICompositePass's cheaper plain per-pixel march.
// Restored here as real Lumen's own "Screen Probe Gather" -- the actual HighQuality GI term now
// that F1 ("Lumen Lite") gives GIMode::Lite its own cheap probe-grid-primary path via
// world_probe_sampling.glsl. The technique was ALREADY probe-sparse before deletion (this class'
// own "Storage" note above: images are sized (m_ProbeCountX, m_ProbeCountY) = renderExtent /
// kProbeTileSize, not full render resolution -- the pre-deletion tombstone's "~10 full-resolution
// images" description was inaccurate), so no resolution change was needed to re-justify it; the one
// real adaptation is RecordGather() no longer read-modify-writing renderer::ClusterResolvePass's
// output color directly (see ScreenProbeGather.comp's own header comment) -- it now writes this
// pass' OWN dedicated indirect-only output image instead, so renderer::GICompositePass can
// composite direct + [this image | ATrous-denoised ScreenTrace] uniformly depending on
// config::lumen::GI_MODE, exactly the way it already composited ScreenTrace's own denoised output
// pre-F9. RecordTrace()/RecordTemporal() keep their exact pre-deletion signatures; only
// RecordGather() dropped its Debug-only `debugViewMode` parameter (see that method's own comment).
...
namespace renderer {
    ...
    public:
        ...
        // F9: this pass' own full-render-resolution indirect-only output image (RecordGather's own
        // write target -- see that method's own comment) -- matches
        // renderer::GICompositePass::kOutputFormat/renderer::ScreenTracePass::kOutputFormat exactly,
        // so GICompositePass can bind whichever GI mode's output uniformly.
        ...
        // Full-resolution bilateral 4-probe gather, writing this pass' OWN output image (see the
        // class comment's F9 note -- no longer a read-modify-write of `resolvePass`'s output color).
        // Every pixel is written unconditionally (including sky/background, as zero) so
        // GetOutputView() is always well-defined for GICompositePass's later read.
    ...
    private:
        ...
        // F9: this pass' own full-render-resolution indirect-only output image -- see
        // GetOutputImage()/GetOutputView()'s own comment and RecordGather()'s own header note.
