...
//     "fait exactement comme dans UE 5.8" scope decision even though this demo authors only one
...
#include "renderer/VirtualShadowMapPool.h"
...
namespace renderer {
    ...
    public:
        ...
        static constexpr uint32_t kSunLevelCount = 3;
        static constexpr float kSunBaseRadius = 2.0f; // Level 0's ortho half-extent; level L = kSunBaseRadius * 2^L.
        ...
        static constexpr uint32_t kPhysicalPageCapacity = 256; // 256 * 128^2 * 4B = 16 MB.
        static constexpr uint32_t kMaxPagesRenderedPerFrame = 32; // Generous: the Fallback Mesh is tiny to render.
