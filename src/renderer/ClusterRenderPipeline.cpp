...
void ClusterRenderPipeline::RecordFrameEarly(VkCommandBuffer cmdEarly,
  ...
  // =========================================================================================
  {
    ...
    // declaration comment -- same clamp-against-stalls formula as that one.
    {
      ...
      // Rivers/waterfalls feature: waterfall mist/foam, a THIRD independent emitter spawned
      // alongside the scene's original ember emitter and the precipitation emitter above -- own
      // fractional accumulator (m_MistSpawnAccumulator), same exact-over-time formula, since it runs
      // at its own rate (config::particles::WATERFALL_SPAWN_RATE_PER_SECOND). Passed into the SAME
      // RecordSimulate() call as a secondary emitter (see that method's own header comment) rather
      // than a second call, so this frame's existing alive particles are integrated exactly once.
