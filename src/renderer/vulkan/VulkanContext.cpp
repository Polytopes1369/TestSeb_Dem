...
void VulkanContext::GenerateGeometry() {
  ...
  // -------------------------------------------------------------------------
  {
    ...
    GenerateBox(cmd, 1.4f, 1.4f, 1.4f, m_EntityData[0].meshID, slot, runningVertexOffset, runningIndexOffset);
  }
  ...
  // -------------------------------------------------------------------------
  {
    ...
    GenerateCone(cmd, 0.7f, 0.35f, 1.4f, m_EntityData[1].meshID, slot, runningVertexOffset, runningIndexOffset);
  }
  ...
  // -------------------------------------------------------------------------
  {
    ...
    GenerateSphere(cmd, 0.8f, m_EntityData[4].meshID, slot, runningVertexOffset, runningIndexOffset);
  }
  ...
  // height and the pivot height would shear the wall sideways instead of just standing it up.
  {
    ...
    // pinned), which is why the wall's OWN world-space X position comes from `slot.x` below.
    {
      ...
      GeneratePlane(cmd, kWallSpan, kWallSpan, m_EntityData[kWallEntityIndexA].meshID, slot,
    ...
    // leaving X and Z pinned) -- perpendicular to Wall A, closing the corner.
    {
      ...
      GeneratePlane(cmd, kWallSpan, kWallSpan, m_EntityData[kWallEntityIndexB].meshID, slot,
                    runningVertexOffset, runningIndexOffset, kWallCenterY,
                    config::FLOOR_VERTEX_SPACING);
    }
  }
  ...
  // -------------------------------------------------------------------------
  {
    ...
    GenerateTerrain(cmd, 300.0f, 300.0f, m_EntityData[kFloorEntityIndex].meshID, slot,
  ...
  // -------------------------------------------------------------------------
  {
    ...
    GenerateWaterPlane(cmd, kWaterPlaneSpan, kWaterPlaneSpan, 2u, 2u, m_EntityData[kWaterEntityIndex].meshID, slot,
  ...
  // -------------------------------------------------------------------------
  {
    ...
    for (uint32_t unit = 0; unit < kStreamingUnitCount; ++unit) {
      ...
      GenerateBox(cmd, 0.6f, 0.6f, 0.6f, m_EntityData[coarseIdx].meshID, coarseSlot, runningVertexOffset, runningIndexOffset);
      ...
      switch (shape) {
        case 0: { // Rock: icosphere.
          ...
          GenerateIcosphere(cmd, 0.5f, false, false, true, m_EntityData[fineIdx].meshID, fineSlot,
                             runningVertexOffset, runningIndexOffset, baseFaceCount, vertsPerFace);
          break;
        }
        case 1: // Bush: UV sphere.
          GenerateSphere(cmd, 0.5f, m_EntityData[fineIdx].meshID, fineSlot, runningVertexOffset, runningIndexOffset);
          break;
        case 2: // Tree: capsule (trunk + canopy silhouette stand-in).
          GenerateCapsule(cmd, 0.25f, 0.8f, m_EntityData[fineIdx].meshID, fineSlot, runningVertexOffset, runningIndexOffset);
          break;
        default: // Debris: torus (irregular scrap silhouette stand-in).
          GenerateTorus(cmd, 0.35f, 0.12f, 0.0f, 0.0f, m_EntityData[fineIdx].meshID, fineSlot, runningVertexOffset, runningIndexOffset);
          break;
      }
    }
  }
