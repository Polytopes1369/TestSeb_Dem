...
// LOCAL COMPATIBILITY PATCH (DemoSceneVK, Phase 7.1 PCG editor-tooling roadmap): mirrors the same
// guard added to this operator's declaration in imgui_extra_math.h -- see that file's own comment
// for the full explanation. Without it this definition collides with imgui.h's own
// `operator*(const float, const ImVec2&)` (already provided once IMGUI_DEFINE_MATH_OPERATORS is
// active, which this header forces on) for any ImGui build >= 19002, a hard duplicate-definition
// compile error -- verified against this project's vendored ImGui 1.92.9 WIP (docking).
# if IMGUI_VERSION_NUM < 19002
